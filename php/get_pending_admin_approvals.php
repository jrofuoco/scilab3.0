<?php
/**
 * get_pending_admin_approvals.php
 * Fetches professor reservations that need admin approval
 */
...
try {
    ...
    error_log("Fetching pending admin approvals for admin ID: " . $adminId);
    
    // Query to get professor reservations pending admin approval
    $query = "
        SELECT 
            r.reservation_id,
            r.reservation_date,
        ...
        WHERE r.professor_approval = 'Approved' AND r.admin_approval = 'Pending' AND u.role = 'Professor'
    ...
    error_log("Found " . count($reservations) . " pending reservations for admin approval");
    error_log("Pending reservations data: " . json_encode($reservations));
    
    // Process and format the data
    $formattedReservations = [];
    foreach ($reservations as $reservation) {
        // Debug: Log each reservation's resources
        error_log("Processing reservation ID: " . $reservation['reservation_id'] . " for professor: " . $reservation['firstname'] . ' ' . $reservation['lastname']);
        
        $formattedReservations[] = [
            'id' => $reservation['reservation_id'],
            'professor_name' => $reservation['firstname'] . ' ' . $reservation['lastname'],
