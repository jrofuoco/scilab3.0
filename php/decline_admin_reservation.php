<?php
/**
 * decline_admin_reservation.php
 * Declines a professor reservation request (admin rejection)
 */
...
try {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        error_log("Admin ID $adminId declined reservation ID $reservationId");
        
        $pdo->commit();
        
        echo json_encode([
            'success' => true,
            'message' => 'Reservation declined successfully. Resources have been restored.'
        ]);
    } else {
        $pdo->rollBack();
        echo json_encode([
    ...
    $pdo->rollBack();
    error_log("Database error in decline_admin_reservation: " . $e->getMessage());
    echo json_encode([
        'success' => false,
